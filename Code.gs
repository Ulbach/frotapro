 ...
 * Compatível com planilha já existente (colunas livres), lendo por NOME DE CABEÇALHO
 * JSONP no GET e POST via <iframe> com callback parent.salvoCallback
 */

// ===== CONFIG =====
const SPREADSHEET_ID = '1InNx25rQ6_5bxqcFK95neE0lF20w-rIHtpyH2QD4vJs'; // <- troque se precisar
...
function colIdx_(map, names){
  for (var i=0;i<names.length;i++){
    const k = norm_(names[i]);
...
function doPost(e){
  ...
  try{
    let payloadText='';
    if (e && e.parameter && typeof e.parameter.postData==='string') payloadText = e.parameter.postData;
    else if (e && e.postData && typeof e.postData.contents==='string') payloadText = e.postData.contents;
    if (!payloadText) throw new Error('Payload ausente');
    if (/^postData=/.test(payloadText)) payloadText = payloadText.replace(/^postData=/,'');
    if (/%7B|%22/i.test(payloadText)) { try{ payloadText = decodeURIComponent(payloadText); }catch(_){} }
...
// ===== Núcleo: leitura do Historico por nome de coluna =====
function getHistorico_(){
  ...
  const rDataSaida   = colIdx_(map,['datasaida','dadossaida']); // aceita seu cabeçalho atual
  ...
  for (var i=0;i<values.length;i++){
    ...
    out.push({
      veiculo,motorista,seguranca,status,destino,
      kmSaida,kmRetorno,kmRodado,dataSaida,dataRetorno,
...
// ===== Gravação respeitando as suas colunas =====
...
// ===== Leitura de Listas (A,B,C) =====
function getListas_(){
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  let sh = ss.getSheetByName('Listas');
  if (!sh) return { veiculos: [], motoristas: [], segurancas: [] };
  const last = sh.getLastRow();
  if (last < 2) return { veiculos: [], motoristas: [], segurancas: [] };
  const rng = sh.getRange(2,1,last-1,3).getValues();
  const v=[], m=[], s=[];
  for (var i=0;i<rng.length;i++){
    const a=String(rng[i][0]||'').trim();
    const b=String(rng[i][1]||'').trim();
    const c=String(rng[i][2]||'').trim();
    if (a) v.push(a); if (b) m.push(b); if (c) s.push(c);
  }
